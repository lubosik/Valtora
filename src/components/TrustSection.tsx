export default function TrustSection() {
  return (
    <section className="section-spacing bg-white">
      <div className="container-custom">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-12">
            <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl text-valtora-navy mb-4 text-balance">
              Trust & Proof
            </h2>
            <p className="text-base sm:text-lg text-gray-700 max-w-2xl mx-auto leading-relaxed px-4">
              See why hundreds of entrepreneurs choose Valtora for their Dubai company formation.
            </p>
          </div>

          {/* Trust Points Grid */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            <div className="text-center p-6 bg-pearl-white rounded-lg">
              <div className="w-16 h-16 bg-emirati-gold/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-emirati-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
              </div>
              <h3 className="font-serif text-xl text-valtora-navy mb-2">
                Registered Partners
              </h3>
              <p className="text-gray-600 text-sm">
                Registered partners with leading UAE free zones including SPC, Meydan, IFZA, DMCC, and more.
              </p>
            </div>

            <div className="text-center p-6 bg-pearl-white rounded-lg">
              <div className="w-16 h-16 bg-emirati-gold/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-emirati-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
              </div>
              <h3 className="font-serif text-xl text-valtora-navy mb-2">
                Dedicated Account Manager
              </h3>
              <p className="text-gray-600 text-sm">
                Dedicated account manager from day one, ensuring personalized support throughout your journey.
              </p>
            </div>

            <div className="text-center p-6 bg-pearl-white rounded-lg">
              <div className="w-16 h-16 bg-emirati-gold/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-emirati-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <h3 className="font-serif text-xl text-valtora-navy mb-2">
                Transparent Pricing
              </h3>
              <p className="text-gray-600 text-sm">
                Transparent, all-inclusive pricing with no hidden fees. What you see is what you pay.
              </p>
            </div>
          </div>

          {/* Testimonials Placeholder */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            <div className="bg-pearl-white p-6 rounded-lg border border-gray-200">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-emirati-gold/20 rounded-full flex items-center justify-center">
                  <span className="text-emirati-gold font-semibold">JD</span>
                </div>
                <div>
                  <p className="font-semibold text-valtora-navy">John D.</p>
                  <p className="text-sm text-gray-600">Tech Startup Founder</p>
                </div>
              </div>
              <p className="text-gray-700 italic">
                &quot;Valtora made the entire process seamless. Clear pricing, expert guidance, and my company was set up faster than expected.&quot;
              </p>
            </div>

            <div className="bg-pearl-white p-6 rounded-lg border border-gray-200">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-emirati-gold/20 rounded-full flex items-center justify-center">
                  <span className="text-emirati-gold font-semibold">SM</span>
                </div>
                <div>
                  <p className="font-semibold text-valtora-navy">Sarah M.</p>
                  <p className="text-sm text-gray-600">E-commerce Business Owner</p>
                </div>
              </div>
              <p className="text-gray-700 italic">
                &quot;The calculator gave me instant clarity on costs. The team was responsive and handled everything professionally.&quot;
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}

